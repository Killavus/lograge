...
module Lograge
  class RequestLogSubscriber < ActiveSupport::LogSubscriber
    def process_action(event)
      ...
      data.merge! extract_params(payload)
    ...
    def extract_params(payload)
      params_cloned = payload[:params].clone
      omit_fields = ['controller', 'action']

      params_cloned.keys.each do |key|
        params_cloned.delete(key) if omit_fields.include?(key) 
      end
